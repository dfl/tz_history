...
class TzHistoryTest < Minitest::Test
  ...
  # Mountain counties now carry a flat MST override (DEFERRED #30, resolved) so IANA's spurious
  # MDT in 1920/1965/1966 is corrected; the war years still defer (IANA = MWT).
  ...
  def test_south_dakota_far_west_mountain_is_mst_not_spurious_mdt
    ...
    assert_nil TzHistory.for(lat:, lon:, date: "1943-07-15") # war -> defer to IANA MWT
    assert_nil TzHistory.for(lat:, lon:, date: "1968-07-15") # post US#1 -> IANA
    # Central SD unchanged (still CST) -- the override only covers the far-west Mountain geometry.
    assert_equal "Etc/GMT+6", TzHistory.for(lat: 44.3683, lon: -100.3510, date: "1965-07-15").identifier # Pierre
  end
