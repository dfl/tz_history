...
| 29 | Pennsylvania | **Cohort nest SHIPPED (2026-09); only the low-town tail remains.** Replaced the pre-war-only PA#1+#6 core with a full NY-style per-table cohort nest (`build_pa.rb`): **33 crop-verified tables** carry their exact EST-summer sets, emitted as a **pre-war + postwar pair of `split` features whose every town holds its own date-ranged `sched`** (the exact spans its table was EST, from America/New_York's DST-transition instants). 4779/6730 towns (71%) corrected pre-war, and the **postwar holdouts** (the deferred value) now ship — PA#6 (1058 towns) EST 1946-1955, PA#7 (256) to 1952, PA#51 (177) to 1947, PA#5/#4/#22/#33/#29/#53/#56/#64/#65 to their own adoption years. Each table read visually from `research/index/PA/tt_reocr/` (persisted) → `research/index/PA/tt_verified.md`. **Crop-verification corrected two stale claims:** PA#6 adopts US#2 in **1956** (not 1965), and PA#9 = **Philadelphia adopts DST 1921** (the OCR-draft `early=[]` was the row-drop the memory warned about). 3 tests. **Still deferred (safe under-correction):** the ~1951 towns on DST cities (correctly defer to IANA) + the unread low-town tail (PA#27=126 ambiguous, #59/#60/#63/#67/#80/#89/#90/#91/#101/#111 and every table <30 towns not yet read). | minority (tail) | low (dominant tables + postwar holdouts now shipped; tail is small/DST) | Extend `TABLE_EST` in `build_pa.rb` by reading the next tier of tables from `research/index/PA/tt_reocr/` (crop-verify each — never trust the OCR text for no-DST). Also re-extract with `--max-table=125` to recover tables 116-123. |
