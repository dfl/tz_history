...
| 29 | Pennsylvania | **Full cohort nest = dedicated follow-up (NY-scale).** Only the crop-verified pre-war core (PA#1+PA#6 rural EST no-DST, 2503 towns, 1919-1942) ships. Deferred: (a) **postwar** — PA#1 had US#2 DST from 1945 (=IANA, correct to defer), but PA#6 kept EST no-DST until 1965 (1058 towns read IANA EDT 1946-1964, under-corrected), and many rural tables adopted DST at staggered years (like NY); (b) the **other ~100 tables** (t7/t8/t13/t51/t114-Pittsburgh-area etc.) are unclassified → defer. **The tt OCR is unreliable** — it reported Philadelphia (PA#9) as no-DST (`early=[]`) though Philly clearly had 1920s-40s DST, so NO table can be asserted from the OCR draft alone; each needs crop-verification. The clean index (6730 towns) enables it. **The re-OCR is PERSISTED (no re-render needed):** `research/index/PA/tt_ocr_cols.txt` = per-column OCR of all 5 tt pages (tables PA#1-123); `research/index/PA/table_class_draft.json` = classifier draft (adoption-year hints only — UNRELIABLE for no-DST, see above). Both are git-ignored + in the Dropbox tarball. | minority / large | medium–high (many towns, but pre-war core + metros already correct/deferred) | Read `tt_ocr_cols.txt` (or re-render the 5 tt pages, ~5 min) to crop-verify each table's EST-year set (like `build_ny.rb`), then extend `build_pa.rb` to a multi-window town split: pre-war for all rural no-DST tables + postwar windows to each table's US#-adoption year (PA#6→1965 is the biggest). Also re-extract with `--max-table=125` to recover tables 116-123. |
