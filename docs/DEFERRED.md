...
| 9 | Illinois | **Town-by-town DST adoption downstate**: larger towns had 1930s CDT (IL #14 etc.); some adopted DST 1946 (IL #7) or across the 1950s (IL #8/#50/#55…) before the 1959 law change. Flat CST override (majority IL #4) over-corrects those town-summers. **Index probe (2026-09):** the IL index votes are too FRAGMENTED for a safe county-level fix — LaSalle's top table is only 13%, and majorities scatter across tables up to #135 (Peoria t98, Sangamon t55, Champaign t54). IL genuinely has 100+ tables and a dense city-listing layout (the one the OCR can't cleanly parse), so the per-city table# is either noisy or the adoption really was town-by-town. **No clean county signal to exclude** → forcing an override change would inject errors. | minority | medium (many towns, decades) | NOT index-driven. Needs the original VISUAL table-classification approach: crop-verify the IL time tables to label each table CST-to-1959 vs early-DST, then per-town splits (discouraged) or accept the minority. Confirmed accepted-minority for now. |
