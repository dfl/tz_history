...
| Pennsylvania | 448 | – | **rural EST no-DST (pre-war core)** | **done*** | NY-scale (6730 towns, 108+ tables, town-by-town DST). **⚠ tt spans PDF 448-452 (5-page span); page 452 is BOTH the last tt page AND the first city page (A-cities)** — old index parsed tt 449-452 as junk. Re-OCR'd (tt 448-452, cities 452bottom-493, `--max-table=115`) → clean index **6730 towns** (real max table ~123; 116-123 dropped by the cap, minor). PA is entirely Eastern; IANA America/New_York bakes continuous EDT into the whole state, but rural PA kept EST no-DST. Crop-verified dominant rural **PA#1 (1445 towns, 21%) = EST no-DST 1887-1942 then US#2 1945** and **PA#6 (1058 towns, 16%) = EST no-DST until US#2 1965** (latest adopter). The DST cities are on their own tables (Philadelphia PA#9, Pittsburgh PA#114, Scranton PA#18, Erie PA#107, Allentown PA#13) → defer. **Shipped the SAFE pre-war core** (`research/builders/build_pa.rb`): a town-level split giving **Etc/GMT+5 to PA#1+PA#6 towns (2503) for 1919-10-26..1942-02-09**; everything else defers. 2 tests. ***Partial (like NY's interim): the full postwar cohort nest (PA#6 EST-to-1965, PA#1 DST-from-1945, and the other ~100 tables' staggered adoption) is a dedicated follow-up — the tt OCR is too noisy to trust per-table (it missed Philadelphia PA#9's EDT rows), so each table needs crop-verification. The clean index now enables it. See DEFERRED.** |
